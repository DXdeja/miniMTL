class MMFrobDisplayWindow extends FrobDisplayWindow;
	...
	if (player != None)
	{
		...
		if (frobTarget != None)
		{
			...
			else if (!frobTarget.bStatic && player.bObjectNames)
			{
				...
				if (frobTarget.IsA('DeusExWeapon') && player.bHelpMessages)
				{
					strInfo = "Skill: " $ class'MMSkillManager'.static.GetSkillGroupByClass(DeusExWeapon(frobTarget).GoverningSkill);
