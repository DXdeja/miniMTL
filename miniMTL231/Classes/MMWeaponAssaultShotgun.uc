class MMWeaponAssaultShotgun extends WeaponAssaultShotgun;
...
{
	local float diff;
	//log("shot time: " $ Level.TimeSeconds);
	diff = Level.TimeSeconds - lastShotTime;
	diff -= 0.39 - ffrtolerance;
	//log("diff: " $ diff);
	if (diff < 0.0)
	{
		//log("too fast fire");
...
defaultproperties
{
     ...
     AmmoNames(0)=Class'MMAmmoShell'
     AmmoName=Class'MMAmmoShell'
}
